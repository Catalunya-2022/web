...
import { getTaskForceLastModified } from "@/lib/content-dates";
  ...
  return (
    <>
      ...
      <JsonLd data={profilePageSchema({
        ...
        sameAs: [
          ...member.links.map((link) => localized(link.url, l)),
          // Every Wikipedia edition plus the Wikidata item, so the entity
          // resolves regardless of which edition a crawler knows.
          ...Object.values(member.wikipedia ?? {}).map((w) => w.url),
          `https://www.wikidata.org/wiki/${member.wikidata}`,
        ],
        locale: l,
        // Same source the sitemap uses, so both freshness signals agree.
        dateModified: getTaskForceLastModified(),
      })} />
      <PageActionBar
        slug={memberSlug}
        locale={l}
      ...
      <div className="flex flex-col-reverse items-center gap-6 sm:flex-row-reverse sm:items-start sm:gap-8">
        ...
        <div className="flex shrink-0 flex-col items-start sm:w-[220px]" data-nosnippet="">
          <Image
          ...
          {hasLinks && (
            <ul className="mx-auto mt-8 w-fit space-y-1.5 text-sm text-muted-foreground">
              {member.links.map((link) => (
                <li key={localized(link.url, l)}>
                  <a
                    href={localized(link.url, l)}
                    target="_blank"
