...
import { titleCaseIdentifier } from "@/lib/content-page-meta";
...
export async function generateMetadata({ params }: Props): Promise<Metadata> {
  ...
  return generateContentArticleMetadata(slug, l);
}
...
export default async function ActionPage({ params }: Props) {
  ...
  return renderArticleContentPage({
    ...
    leadingBreadcrumbs: [
      ...
      {
        name: titleCaseIdentifier(manifest.entries.get(spherePath)?.identifier ?? `${t.sphere} ${sphereId}`),
        url: buildAbsoluteUrl(spherePath, l),
      },
      {
        name: titleCaseIdentifier(manifest.entries.get(goalPath)?.identifier ?? `${t.goal} ${goalId}`),
