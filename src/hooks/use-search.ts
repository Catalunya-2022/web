...
export function useSearch(locale: Locale, options?: UseSearchOptions): UseSearchReturn {
  ...
  useEffect(() => {
    ...
    // Warm up at browser idle so the corpus fetch and index build don't
    // compete with hydration: this hook mounts on every page via TopBar's
    // always-rendered SearchDialog. Safari lacks requestIdleCallback.
    let idleId: number | undefined;
    let timerId: ReturnType<typeof setTimeout> | undefined;
    if (typeof requestIdleCallback === "function") {
      idleId = requestIdleCallback(() => warmUp());
    } else {
      timerId = setTimeout(warmUp, 200);
    }
  ...
  }, [query, locale, scopeKey, retryNonce]);
