...
import { normalizeForSearch, stripDiacritics } from "@/lib/search-normalization";
...
function extractSnippet(
  ...
  const lower = stripDiacritics(body);
  // Offsets found on the normalized string are applied to the raw body, so
  // they are only valid while the two stay the same length (true for NFC).
  if (lower.length !== body.length) {
    return body.slice(0, windowSize) + (body.length > windowSize ? "..." : "");
  }
