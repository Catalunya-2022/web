...
describe("resolveLocalizedRoute", () => {
  ...
  it("returns null for unknown and locale-mismatched paths", () => {
    ...
    // by a separate branch (isCaHierarchyPath). If resolveLocalizedRoute
...
describe("isCaHierarchyPath", () => {
  it("matches pathless CA hierarchy paths", () => {
    expect(isCaHierarchyPath("/ambit-1")).toBe(true);
    expect(isCaHierarchyPath("/ambit-2/objectiu-7")).toBe(true);
  });

  it("rejects EN/ES hierarchy prefixes (they always arrive locale-prefixed)", () => {
    expect(isCaHierarchyPath("/sphere-2")).toBe(false);
    expect(isCaHierarchyPath("/sphere-1/goal-4/action-4-3")).toBe(false);
    expect(isCaHierarchyPath("/ambito-3")).toBe(false);
    expect(isCaHierarchyPath("/ambito-1/objetivo-4/accion-4-3")).toBe(false);
  });

  it("rejects non-hierarchy paths", () => {
    expect(isCaHierarchyPath("/task-force")).toBe(false);
    expect(isCaHierarchyPath("/grup-de-treball")).toBe(false);
    expect(isCaHierarchyPath("/introduccio")).toBe(false);
    expect(isCaHierarchyPath("/")).toBe(false);
    expect(isCaHierarchyPath("")).toBe(false);
  });

  // CRITICAL — locale-prefixed paths must not match. Breaking this would
  // cause infinite rewrite loops (the proxy would intercept /en/sphere-1,
  // rewrite to /ca/en/sphere-1, re-enter, ...).
  it("[critical] rejects locale-prefixed paths", () => {
    expect(isCaHierarchyPath("/en/sphere-1")).toBe(false);
    expect(isCaHierarchyPath("/en/sphere-1/goal-4/action-4-3")).toBe(false);
    expect(isCaHierarchyPath("/es/ambito-1")).toBe(false);
    expect(isCaHierarchyPath("/es/ambito-1/objetivo-4/accion-4-3")).toBe(false);
  });
  ...
  it("matches prefix even for out-of-range IDs (404 at page level)", () => {
    expect(isCaHierarchyPath("/ambit-99")).toBe(true);
  });
});
...
describe("resolveRoute", () => {
  ...
  it("returns null for non-CA hierarchy without locale prefix (no legacy redirects)", () => {
    expect(resolveRoute("/sphere-1", "ca")).toBeNull();
    expect(resolveRoute("/sphere-2/goal-6/action-6-3", "ca")).toBeNull();
    expect(resolveRoute("/ambito-1", "ca")).toBeNull();
    expect(resolveRoute("/ambito-1/objetivo-4/accion-4-3", "ca")).toBeNull();
  });
