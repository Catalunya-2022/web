...
/** Longest complete-sentence prefix within maxLen, or null when the last
 *  sentence boundary falls too early to make a useful description. */
export function truncateAtSentenceBoundary(
  ...
  if (text.length <= maxLen) return text;
  const match = text.slice(0, maxLen + 1).match(/^[\s\S]*[.!?](?=\s)/);
  if (!match) return null;
  const sentence = match[0].trimEnd();
  return sentence.length >= minLen ? sentence : null;
}
