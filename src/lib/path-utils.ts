  ...
  for (const parsed of PARSED_CATALOG) {
    ...
    if (params !== null) {
      const localePattern = Object.entries(params).reduce(
        // Replacer function: a plain string replacement would expand $-patterns
        // in values that come from the request path.
...
/** CA hierarchy segment prefixes (ambit, objectiu, accio). The default locale
 *  is pathless, so CA hierarchy paths are the only unprefixed ones the proxy
 *  rewrites; EN/ES hierarchy paths always arrive locale-prefixed. */
const CA_HIERARCHY_PREFIXES: ReadonlySet<string> = new Set(
  ALL_PREFIXES.filter(({ locale }) => locale === "ca").map(({ prefix }) => prefix)
);

/** True for pathless CA hierarchy paths (ambit-1, objectiu-2, accio-2-1 segments). */
export function isCaHierarchyPath(pathname: string): boolean {
  const firstSeg = pathname.split("/").filter(Boolean)[0];
  if (!firstSeg) return false;
  const dashIdx = firstSeg.indexOf("-");
  if (dashIdx === -1) return false;
  return CA_HIERARCHY_PREFIXES.has(firstSeg.slice(0, dashIdx));
}
 ...
 *  → rewriteToInternal with raw pathname. */
 ...
 *  of the handlers or returns null to fall through to intlMiddleware. Callers
