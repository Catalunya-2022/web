...
import { ExternalLink, Link as LinkIcon, Printer, Quote, Share2 } from "lucide-react";
...
export function useCopyPageActions(
  ...
  const printTimerRef = useRef<ReturnType<typeof setTimeout>>(null);
  ...
  const handleCopy = useCallback(() => {
    // navigator.clipboard is undefined in non-secure contexts; touching
    // .writeText there throws synchronously, before any .catch exists.
    if (!payload || copyState === "copying" || !navigator.clipboard) return;
    trackEvent("Copy Page: Markdown");
  ...
  const handleShareOrCopyLink = useCallback(async () => {
    if (!payload) return;
    trackEvent("Copy Page: Share Link");
    if (canShare) {
  ...
  const handlePrint = useCallback(() => {
    trackEvent("Copy Page: Print");
    printTimerRef.current = setTimeout(() => window.print(), 150);
  }, []);
    ...
    <DropdownMenuContent align="end" collisionPadding={8} className="w-72 max-w-[calc(100vw-1rem)] p-1.5">
      ...
      <DropdownMenuItem asChild>
        <a
          ...
          onClick={() => trackEvent("Copy Page: ChatGPT")}
          className="flex items-center gap-3 rounded-md px-3 py-2.5"
        >
          <span className="flex size-8 shrink-0 items-center justify-center rounded-md border border-border bg-background">
      ...
      <DropdownMenuItem asChild>
        <a
          ...
          target="_blank"
          rel="noopener noreferrer"
          onClick={() => trackEvent("Copy Page: Claude")}
          className="flex items-center gap-3 rounded-md px-3 py-2.5"
        >
          <span className="flex size-8 shrink-0 items-center justify-center rounded-md border border-border bg-background">
