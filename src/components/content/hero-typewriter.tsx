...
export function HeroTypewriter({ items, locale }: { items: HeroItem[]; locale: Locale }) {
  ...
  // Announce the hero line once; re-announcing every rotation would make the
  // live region interrupt screen readers indefinitely.
  useEffect(() => {
    if (!mounted || phase !== "holding" || announcement) return;
    const raf = requestAnimationFrame(() =>
      setAnnouncement(`RESET: ${fullText}`)
    );
    return () => cancelAnimationFrame(raf);
  }, [mounted, phase, announcement, fullText]);
