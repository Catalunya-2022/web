...
// A NaN or non-positive batch size would end the render loop after one pass
// and exit 0 with an emptied public/og, so refuse anything invalid.
const BATCH_SIZE = (() => {
  const raw = process.env.OG_BATCH_SIZE;
  if (raw === undefined) return 8;
  const parsed = Number.parseInt(raw, 10);
  if (!Number.isInteger(parsed) || parsed <= 0) {
    throw new Error(`Invalid OG_BATCH_SIZE: ${JSON.stringify(raw)}`);
  }
  return parsed;
})();
...
function loadHeadings(
  ...
  try {
    ...
    return { h1: stripMarkdownFormatting(h1), h2: stripMarkdownFormatting(h2) };
  } catch (e) {
    throw new Error(
...
async function main() {
  ...
  // Load the font once and cache photos once (30 reads, not 90)
  const fontData = await getFontData();
  const photoCache = await loadAllPhotos();

  const specs = buildAllSpecs(photoCache);
  // Derived from the same slug sources buildAllSpecs walks, so adding a page
  // updates guard and specs together (was a hardcoded 147).
  const routesPerLocale =
    1 + // home
    STATIC_CONTENT_SLUGS.length +
    SUPPLEMENTARY_PAGE_SLUGS.length +
    getSphereIds().length +
    getGoalRoutes().length +
    getActionRoutes().length +
    teamMembers.length;
  const expectedCount = routesPerLocale * LOCALES.length;
  ...
  if (count !== specs.length) {
    throw new Error(
      `[og-images] rendered ${count} of ${specs.length} images; output is incomplete`,
    );
  }
